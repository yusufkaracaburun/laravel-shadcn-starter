...
test('authenticated user can list their teams', function () {
    // Arrange
...
test('authenticated user can update their team', function () {
    // Arrange
    $user = User::factory()->create();
    $team = Team::factory()->create(['user_id' => $user->id]);
    Sanctum::actingAs($user);
...
test('user cannot update team they do not own', function () {
    // Arrange
...
test('authenticated user can delete their team', function () {
    // Arrange
    $user = User::factory()->create();
    $team = Team::factory()->create(['user_id' => $user->id]);
    Sanctum::actingAs($user);
...
test('authenticated user can switch current team', function () {
    // Arrange
    $user = User::factory()->create();
    $team1 = Team::factory()->create(['user_id' => $user->id]);
    $team2 = Team::factory()->create();
