...
function convertDocument(docs, { directives, doc, node, docEnd }, ctx, parent, startIndex) {
    if (!doc || !node) {
        ...
        const ast = Object.assign({ type: "YAMLDocument", directives: [], content: null, parent, anchors: {}, version: docs.streamInfo.directives.yaml.version }, loc);
...
function convertDirective(node, cst, ctx, parent) {
    ...
    let ast;
    if (name === "%YAML") {
        ast = Object.assign({ type: "YAMLDirective", value, kind: "YAML", version: node ? node.yaml.version : cst.source.slice(6).trim(), parent }, loc);
    }
    else if (name === "%TAG") {
