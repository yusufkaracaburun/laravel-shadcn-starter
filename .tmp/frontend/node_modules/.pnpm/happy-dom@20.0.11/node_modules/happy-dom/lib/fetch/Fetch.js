...
export default class Fetch {
    ...
    async send() {
        if (!(this.request instanceof Request) || !(this.request[PropertySymbol.url] instanceof URL)) {
            throw new this.#window.DOMException('Unknown request object. Request object must be an instance of Request.', DOMExceptionNameEnum.invalidStateError);
        }
        ...
        if (this.request.signal[PropertySymbol.aborted]) {
            ...
            throw new this.#window.DOMException('signal is aborted without reason', DOMExceptionNameEnum.abortError);
        }
        ...
        if (this.request[PropertySymbol.url].protocol === 'http:' &&
            ...
            throw new this.#window.DOMException(`Mixed Content: The page at '${this.#window.location.href}' was loaded over HTTPS, but requested an insecure XMLHttpRequest endpoint '${this.request.url}'. This request has been blocked; the content must be served over HTTPS.`, DOMExceptionNameEnum.securityError);
        }
        ...
        if (!this.disableSameOriginPolicy) {
            ...
            if (!compliesWithCrossOriginPolicy) {
                ...
                throw new this.#window.DOMException(`Cross-Origin Request Blocked: The Same Origin Policy disallows reading the remote resource at "${this.request.url}".`, DOMExceptionNameEnum.networkError);
            }
        }
    ...
    async getVirtualServerResponse() {
        ...
        response[PropertySymbol.cachedResponse] = this.#browserFrame.page.context.responseCache.add(this.request, cacheableResponse);
    ...
    onSocket(socket) {
        const onSocketClose = () => {
            if (this.isChunkedTransfer && !this.isProperLastChunkReceived) {
                const error = new this.#window.DOMException('Premature close.', DOMExceptionNameEnum.networkError);
    ...
    onError(error) {
        ...
        this.reject(new this.#window.DOMException(`Failed to execute "fetch()" on "Window" with URL "${this.request.url}": ${error.message}`, DOMExceptionNameEnum.networkError));
    }
    /**
     * Triggered when the async task manager aborts.
     */
    onAsyncTaskManagerAbort() {
        const error = new this.#window.DOMException('The operation was aborted.', DOMExceptionNameEnum.abortError);
    ...
    onResponse(nodeResponse) {
        ...
        if (this.request.method === 'HEAD' ||
            ...
            this.response = new this.#window.Response(FetchBodyUtility.nodeToWebStream(body), responseOptions);
    ...
    handleRedirectResponse(nodeResponse, responseHeaders) {
        ...
        switch (this.request.redirect) {
            case 'error':
                this.finalizeRequest();
                this.reject(new this.#window.DOMException(`URI requested responds with a redirect, redirect mode is set to "error": ${this.request.url}`, DOMExceptionNameEnum.abortError));
                return true;
            case 'manual':
                // Nothing to do
                return false;
            case 'follow':
                ...
                if (locationHeader !== null) {
                    try {
                        ...
                        this.finalizeRequest();
                        this.reject(new this.#window.DOMException(`URI requested responds with an invalid redirect URL: ${locationHeader}`, DOMExceptionNameEnum.uriMismatchError));
                        return true;
                    }
                }
                if (locationURL === null) {
                ...
                if (FetchResponseRedirectUtility.isMaxRedirectsReached(this.redirectCount)) {
                    this.finalizeRequest();
                    this.reject(new this.#window.DOMException(`Maximum redirects reached at: ${this.request.url}`, DOMExceptionNameEnum.networkError));
                    return true;
                }
     ...
     */
    abort(reason) {
        const error = new this.#window.DOMException('The operation was aborted.' + (reason ? ' ' + reason.toString() : ''), DOMExceptionNameEnum.abortError);
        this.request[PropertySymbol.aborted] = true;
        this.request[PropertySymbol.error] = error;
        if (this.response) {
            this.response[PropertySymbol.aborted] = true;
