...
export default class SyncFetch {
    ...
    getCachedResponse() {
        ...
        return {
            ...
            [PropertySymbol.virtualServerFile]: cachedResponse.response[PropertySymbol.virtualServerFile] || null
        };
    }
    /**
    ...
    sendRequest() {
        if (!this.request[PropertySymbol.bodyBuffer] && this.request.body) {
            throw new this.#window.DOMException(`Streams are not supported as request body for synchronous requests.`, DOMExceptionNameEnum.notSupportedError);
        }
        ...
        if (!content.length) {
            throw new this.#window.DOMException(`Synchronous fetch to "${this.request.url}" failed.`, DOMExceptionNameEnum.networkError);
        }
        const { error, incomingMessage } = JSON.parse(content.toString());
        if (error) {
            throw new this.#window.DOMException(`Synchronous fetch to "${this.request.url}" failed. Error: ${error}`, DOMExceptionNameEnum.networkError);
        }
    ...
    parseIResponseBody(options) {
        ...
        try {
            ...
            throw new this.#window.DOMException(`Failed to read response body. Error: ${error.message}.`, DOMExceptionNameEnum.encodingError);
        }
        return options.body;
    }
    /**
    ...
    handleRedirectResponse(response) {
        ...
        switch (this.request.redirect) {
            case 'error':
                throw new this.#window.DOMException(`URI requested responds with a redirect, redirect mode is set to "error": ${this.request.url}`, DOMExceptionNameEnum.abortError);
            case 'manual':
                return null;
            case 'follow':
                ...
                if (locationHeader !== null) {
                    try {
                        ...
                        throw new this.#window.DOMException(`URI requested responds with an invalid redirect URL: ${locationHeader}`, DOMExceptionNameEnum.uriMismatchError);
                    }
                }
                if (locationURL === null) {
                    return null;
                ...
                if (FetchResponseRedirectUtility.isMaxRedirectsReached(this.redirectCount)) {
                    throw new this.#window.DOMException(`Maximum redirects reached at: ${this.request.url}`, DOMExceptionNameEnum.networkError);
                }
