...
test.describe('Authentication Flow', () => {
  ...
  test('User can complete full authentication flow (register → login → logout)', async ({ When, Then, And, page }) => { 
    await When('the user navigates to the register page', null, { page }); 
    await And('the user fills in name "Test User", email "flowuser@example.com", password "password123", and password confirmation "password123"', null, { page }); 
    ...
    await When('the user fills in email "flowuser@example.com" and password "password123"', null, { page }); 
  ...
  test('User can access protected routes after login', async ({ Given, When, Then, And, page, request }) => { 
    ...
    await And('the user fills in email "protected@example.com" and password "password123"', null, { page }); 
    await And('the user submits the login form', null, { page }); 
    await Then('the user should be on the dashboard page', null, { page }); 
    await When('the user navigates to the dashboard directly', null, { page }); 
    await Then('the user should be on the dashboard page', null, { page }); 
  });

  test('User is redirected to login when accessing protected routes without auth', async ({ When, Then, And, page }) => { 
    await When('the user navigates to the dashboard directly', null, { page }); 
    await Then('the user should be redirected to the login page', null, { page }); 
    await And('the user should not be authenticated', null, { page }); 
  });

  test('User session persists after page reload', async ({ Given, When, Then, And, page, request }) => { 
    await Given('a user is registered with email "session@example.com" and password "password123"', null, { request }); 
    await When('the user navigates to the login page', null, { page }); 
    await And('the user fills in email "session@example.com" and password "password123"', null, { page }); 
    await And('the user submits the login form', null, { page }); 
    await Then('the user should be on the dashboard page', null, { page }); 
    await When('the user reloads the page', null, { page }); 
    await Then('the user should still be on the dashboard page', null, { page }); 
    await And('the user should still be authenticated', null, { page }); 
  });
