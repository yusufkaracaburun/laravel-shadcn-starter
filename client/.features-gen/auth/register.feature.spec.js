...
test.describe('User Registration', () => {
  ...
  test('User can register successfully with valid data', async ({ When, Then, And, loginPage, page, registerPage }) => { 
    await When('the user navigates to the register page', null, { registerPage }); 
    await And('the user fills in name "Test User", email "newuser@example.com", password "password123", and password confirmation "password123"', null, { registerPage }); 
    ...
    await And('the user should be able to login with email "newuser@example.com" and password "password123"', null, { loginPage, page }); 
  });

  test('User cannot register with duplicate email', async ({ Given, When, Then, And, page, registerPage, request }) => { 
    await Given('a user is registered with email "existing@example.com" and password "password123"', null, { request }); 
    await When('the user navigates to the register page', null, { registerPage }); 
    await And('the user fills in name "Another User", email "existing@example.com", password "password123", and password confirmation "password123"', null, { registerPage }); 
    await And('the user submits the registration form', null, { page, registerPage }); 
  ...
  test('User is redirected to login after successful registration', async ({ When, Then, And, loginPage, page, registerPage }) => { 
    await When('the user navigates to the register page', null, { registerPage }); 
    await And('the user fills in name "Test User", email "newuser2@example.com", password "password123", and password confirmation "password123"', null, { registerPage }); 
    await And('the user submits the registration form', null, { page, registerPage }); 
    await Then('the user should be on the login page', null, { loginPage }); 
  });
