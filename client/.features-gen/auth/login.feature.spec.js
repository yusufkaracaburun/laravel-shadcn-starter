...
test.describe('User Login', () => {
  ...
  test('User can login successfully with valid credentials', async ({ Given, When, Then, And, dashboardPage, loginPage, page, request }) => { 
    ...
    await And('the user fills in email "test@example.com" and password "password123"', null, { loginPage }); 
    await And('the user submits the login form', null, { loginPage, page }); 
    await Then('the user should be redirected to the dashboard', null, { dashboardPage, page }); 
    await And('the user should be authenticated', null, { dashboardPage }); 
  });

  test('User cannot login with invalid credentials', async ({ When, Then, And, loginPage, page }) => { 
    await When('the user navigates to the login page', null, { loginPage }); 
    await And('the user fills in email "invalid@example.com" and password "wrongpassword"', null, { loginPage }); 
    await And('the user submits the login form', null, { loginPage, page }); 
    await Then('the user should see an error message', null, { page }); 
    await And('the user should remain on the login page', null, { page }); 
  });

  test('User is redirected to dashboard after successful login', async ({ Given, When, Then, And, dashboardPage, loginPage, page, request }) => { 
    await Given('a user is registered with email "test@example.com" and password "password123"', null, { request }); 
    await When('the user navigates to the login page', null, { loginPage }); 
    await And('the user fills in email "test@example.com" and password "password123"', null, { loginPage }); 
    await And('the user submits the login form', null, { loginPage, page }); 
