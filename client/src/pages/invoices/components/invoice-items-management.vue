...
<template>
  <div class="space-y-4">
    ...
    <div v-if="showAddForm || editingItemIndex !== null" class="rounded-lg border p-4">
      <InvoiceItemForm
        :item="editingItemIndex !== null && editingItemIndex >= 0 ? (items[editingItemIndex] as any) || null : null"
        @save="handleSave"
        @cancel="handleCancel" />
    </div>

    <!-- Items Table -->
    <div v-if="items.length === 0 && !showAddForm"
    ...
    <div v-else-if="items.length > 0" class="space-y-2">
      ...
      <InvoiceTotalsSummary :subtotal="invoiceTotals.subtotal" :total-vat0="invoiceTotals.totalVat0"
        :total-vat9="invoiceTotals.totalVat9" :total-vat21="invoiceTotals.totalVat21"
